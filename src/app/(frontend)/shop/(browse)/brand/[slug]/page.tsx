...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  const brand = await getCachedDocument<typeof BRAND_SLUG>(BRAND_SLUG, slug)
  if (!brand) {
    notFound()
  }
  return {
    title: `${brand.name} | BonaVista Leisurescapes`,
    description: `Shop products from ${brand.name}`,
    openGraph: mergeOpenGraph({
      title: `${brand.name} | BonaVista Leisurescapes`,
      description: `Shop products from ${brand.name}`,
      url: `${process.env.NEXT_PUBLIC_PUBLIC_URL}/shop/brand/${slug}`,
    }),
  }
}
...
export default async function BrandPage({ params, searchParams }: Props) {
  ...
  const brand = await getCachedDocument<typeof BRAND_SLUG>(BRAND_SLUG, slug, 2)
