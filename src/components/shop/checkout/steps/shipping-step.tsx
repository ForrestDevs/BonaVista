...
export function ShippingStep({
  ...
  const [address, setAddress] = useState<StripeAddress | null>(initialAddress || null)
  ...
  }, [session.steps.shipping.method?.id])
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault()
    if (!elements || !address || !selectedMethod || isProcessing || isDisabled || isLoading) return
  ...
  return (
    <div className={`space-y-6 ${isDisabled ? 'opacity-60 pointer-events-none' : ''}`}>
      ...
      <Button
        type="button"
        className="w-full"
        disabled={isProcessing || !address || !selectedMethod || isDisabled || isLoading}
        onClick={handleSubmit}
      >
        {isProcessing || isLoading ? 'Processing...' : 'Continue to Billing'}
      </Button>
    </div>
  )
}
