...
export function ProductCard({ product }: { product: Product }) {
  const [selectedVariant, setSelectedVariant] = useState<ProductVariants | null>(
    product.variants && product.variants.length > 0 ? product.variants[0] as ProductVariants : null
  )

  const handleAddToCart = (e: React.MouseEvent) => {
    e.preventDefault() // Prevent navigation to product detail page
    if (selectedVariant) {
      console.log(`Added to cart: ${product.title} - ${selectedVariant.size}`)
    ...
    <Link href={`/store/product/${product.id}`} passHref>
      <Card className="cursor-pointer group transition-all duration-300 hover:shadow-lg">
        ...
        <CardContent className="p-4">
          ...
          <p className="text-sm text-muted-foreground mt-2">{product.meta?.description}</p>
          {selectedVariant && (
